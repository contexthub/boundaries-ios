...
@interface BDYListTableViewController ()
@property (nonatomic, weak) BDYGeofence *selectedGeofence;
...
@implementation BDYListTableViewController
        ...
        [[CCHGeofenceService sharedInstance] updateGeofence:geofenceDict completionHandler:^(NSError *error) {
            ...
            if (!error) {
                ...
                [[CCHSensorPipeline sharedInstance] synchronize:^(NSError *error) {
                    ...
                    if (!error) {
                        NSLog(@"BDY: Successfully updated and synchronized geofence %@ on ContextHub", self.selectedGeofence.name);
                        [self.tableView reloadData];
                    } else {
                        NSLog(@"BDY: Could not synchronize update of geofence %@ on ContextHub", self.selectedGeofence.name);
                    }
                }];
            } else {
                ...
                NSLog(@"BDY: Could not update geofence %@ on ContextHub", self.selectedGeofence.name);
            }
        }];
    }
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    BDYGeofenceCell *cell = [tableView dequeueReusableCellWithIdentifier:@"BDYGeofenceCellIdentifier"];
    BDYGeofence *geofence = self.geofenceArray[indexPath.row];
...
- (void)tableView:(UITableView *)tableView commitEditingStyle:(UITableViewCellEditingStyle)editingStyle forRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (editingStyle == UITableViewCellEditingStyleDelete) {
        // Delete a geofence
        BDYGeofence *geofenceToDelete = self.geofenceArray[indexPath.row];
        ...
        [[CCHGeofenceService sharedInstance] deleteGeofence:geofenceDict completionHandler:^(NSError *error) {
            if (!error) {
                ...
                [[CCHSensorPipeline sharedInstance] synchronize:^(NSError *error) {
                    ...
                    if (!error) {
                        NSLog(@"BDY: Successfully deleted geofence %@ on ContextHub", geofenceToDelete.name);
                        ...
                        NSLog(@"BDY: Could not synchronize deletion of geofence %@ on ContextHub", geofenceToDelete.name);
                    }
                ...
                NSLog(@"BDY: Could not delete geofence %@ on ContextHub", geofenceToDelete.name);
            }
        }];
    }
}
